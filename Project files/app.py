# app.py
import streamlit as st
from dotenv import load_dotenv
import os
import google.generativeai as genai
from PIL import Image

# --- Load Environment Variables ---
load_dotenv()
api_key = os.getenv("GEMINI_API_KEY")

if api_key:
    genai.configure(api_key=api_key)
else:
    st.error("❌ GEMINI_API_KEY not found! Add it to your .env file")
    st.stop()

# --- Streamlit Page Setup ---
st.set_page_config(
    page_title="🏺 Gemini Artifact Insight AI",
    page_icon="🏺",
    layout="centered"
)

# --- Custom CSS ---
st.markdown("""
    <style>
        .stApp {background-color: #ecf0f3; font-family: 'Verdana', sans-serif; color: #222222;}
        .title {font-family: 'Georgia', serif; font-size: 36px; font-weight: 700; color: black; text-align: center;}
        .stButton button {background-color: #4B47C3; color: white; border-radius: 8px;}
        .stButton button:hover {background-color: #3934a1;}
    </style>
""", unsafe_allow_html=True)

# --- Header ---
st.markdown('<div class="title">🏺 Gemini Artifact Insight AI</div>', unsafe_allow_html=True)
st.write("Generate interesting historical facts and analysis of Ancient India artifacts using AI.")

# --- Sidebar for Uploads & Input ---
with st.sidebar:
    st.header("📥 Upload Inputs")
    uploaded_file = st.file_uploader("Upload Artifact Image", type=["jpg", "jpeg", "png"])
    input_text = st.text_area(
        "📌 Add Context (optional)", 
        placeholder="What do you want to know about this artifact?"
    )
    submit = st.button("🔍 Generate Description")

# --- Show uploaded image ---
if uploaded_file:
    st.image(uploaded_file, caption="🖼 Uploaded Artifact", use_container_width=True)

# --- Utility function to prepare image for Gemini API ---
def input_image_setup(uploaded_file):
    if uploaded_file is not None:
        bytes_data = uploaded_file.getvalue()
        return [{"mime_type": uploaded_file.type, "data": bytes_data}]
    return None

# --- Generate response from Gemini ---
def get_gemini_response(input_text, image_data, prompt):
    full_prompt = f"{prompt}\n\nUser Instruction: {input_text or 'Describe this artifact'}"
    model = genai.GenerativeModel("gemini-2.5-flash")  # ✅ Working model

    try:
        if image_data:
            response = model.generate_content([full_prompt, image_data[0]])
        else:
            response = model.generate_content(full_prompt)
        return response.text
    except Exception as e:
        return f"❌ Error generating response: {str(e)}"

# --- System Prompt for AI ---
system_prompt = """
You are an expert historian. Analyze the artifact image and provide:
- Name/identification
- Historical period/origin  
- Materials used
- Cultural significance
- Interesting facts
Keep response detailed but engaging (300-500 words).
"""

# --- Generate AI Analysis ---
if submit:
    if not uploaded_file:
        st.warning("⚠️ Please upload an artifact image first!")
    else:
        with st.spinner("🔍 AI is analyzing your artifact..."):
            image_data = input_image_setup(uploaded_file)
            response = get_gemini_response(input_text, image_data, system_prompt)

        st.success("✅ Analysis complete!")
        st.markdown("### 📜 Artifact Analysis")
        st.markdown(response)

        # --- Download button ---
        st.download_button(
            label="📥 Download Report",
            data=response,
            file_name="artifact_report.txt",
            mime="text/plain"
        )

# --- Footer ---
st.markdown("---")
st.markdown(
    "<p style='text-align: center; color: #666;'>Powered by Gemini 2.5 Flash • For SmartInternz Project</p>", 
    unsafe_allow_html=True
)
